(function ($) {
  ...
  function updateLogoPreview(url) {
    ...
    if (url) {
      preview.html('<img src="' + url + '" alt="Product logo preview" />');
    } else {
      preview.html(
  ...
  function updateLogsHiddenField() {
    var logs = [];
    $("#main-product-update-logs-container .main-product-meta__update-log-item").each(function () {
      var $item = $(this);
      var date = $item.find(".main-product-update-log-date").val().trim();
      var description = $item.find(".main-product-update-log-description").val().trim();
  ...
  function updateMoveButtons() {
    $("#main-product-update-logs-container .main-product-meta__update-log-item").each(function (index) {
      var $item = $(this);
      var total = $("#main-product-update-logs-container .main-product-meta__update-log-item").length;

      $item.find(".main-product-update-log-move-up").prop("disabled", index === 0);
      $item.find(".main-product-update-log-move-down").prop("disabled", index === total - 1);
    });
  }
  ...
  function bindUpdateLogs() {
    ...
    $(document).on("click", ".main-product-update-log-remove", function (event) {
      event.preventDefault();
      removeUpdateLog($(this).closest(".main-product-meta__update-log-item"));
    });

    $(document).on("click", ".main-product-update-log-move-up", function (event) {
      event.preventDefault();
      var $item = $(this).closest(".main-product-meta__update-log-item");
      moveUpdateLog($item, "up");
    });

    $(document).on("click", ".main-product-update-log-move-down", function (event) {
      event.preventDefault();
      var $item = $(this).closest(".main-product-meta__update-log-item");
  ...
  function updateScoreBreakdownHiddenField() {
    var criteria = [];
    $("#main-product-score-breakdown-container .main-product-meta__score-criterion").each(function () {
      var $item = $(this);
      var name = $item.find(".main-product-score-criterion-name").val().trim();
      var score = parseFloat($item.find(".main-product-score-criterion-score").val()) || 0;
  ...
  function calculateAndUpdateRating() {
    ...
    $("#main-product-score-breakdown-container .main-product-meta__score-criterion").each(function () {
      var score = parseFloat($(this).find(".main-product-score-criterion-score").val()) || 0;
      var name = $(this).find(".main-product-score-criterion-name").val().trim();
    ...
    if (count > 0) {
      ...
      $("#main-product-rating").val(average);
    } else {
      $("#main-product-auto-rating").text("N/A");
      $("#main-product-rating").val("");
    }
  }
  ...
  function addScoreCriterion() {
    ...
    var $item = $(
      '<div class="main-product-meta__score-criterion" data-index="' + index + '">' +
  ...
  function bindScoreBreakdown() {
    $(document).on("click", ".main-product-add-score-criterion", function (event) {
      event.preventDefault();
      addScoreCriterion();
    });

    $(document).on("click", ".main-product-score-criterion-remove", function (event) {
      event.preventDefault();
      removeScoreCriterion($(this).closest(".main-product-meta__score-criterion"));
    });
