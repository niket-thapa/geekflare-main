...
if ( ! empty( $sticky_post_ids ) && is_array( $sticky_post_ids ) ) {
	// Get sticky posts that belong to selected categories
				...
				<?php if ( ! empty( $sticky_posts ) ) : ?>
					...
					>
						<?php foreach ( $sticky_posts as $post ) : ?>
							...
							?>
							<article class="w-full flex flex-col gap-6 p-6">
								...
								<div class="flex flex-col gap-4">
									...
									<?php endif; ?>
									<a href="<?php echo esc_url( get_permalink( $post->ID ) ); ?>" class="text-sm font-semibold text-primary hover:text-primary-hover py-2 md:py-3">
										Read Guide
									</a>
								</div>
							</article>
					...
					<?php foreach ( $regular_posts as $index => $post ) : ?>
						...
						<article class="flex flex-col gap-4<?php echo $is_last ? '' : ' border-b border-gray-200 pb-8'; ?>">
							...
							<div class="flex flex-col gap-2">
								<h3 class="text-lg md:text-xl md:-tracking-2p font-semibold text-gray-900 line-clamp-1">
									<a href="<?php echo esc_url( get_permalink( $post->ID ) ); ?>">
										<?php echo esc_html( get_the_title( $post->ID ) ); ?>
									</a>
								</h3>
							...
							<a href="<?php echo esc_url( get_permalink( $post->ID ) ); ?>" class="text-sm font-semibold text-primary hover:text-primary-hover md:py-2 md:mt-1">
								Read Guide
							</a>
						</article>
					<?php endforeach; ?>
