...
(function (blocks, blockEditor, element, components, i18n) {
  ...
  blocks.registerBlockType("main/meet-experts", {
    edit: function (props) {
      ...
      var subheading = attributes.subheading || "Discover in-depth insights, reviews, and guides crafted by our team of industry experts.";
      ...
      var getAuthorData = function(authorId) {
        ...
        return authorsCache.find(function(a) {
      ...
      return el(
          ...
          el(
            ...
            authors.length > 0 &&
              authors.map(function (authorId, index) {
                ...
                var authorName = author ? (author.name || author.slug || "Unknown") : "Loading...";
                return el(
                  "div",
                  {
                    key: index,
        ...
        el(
          "section",
          { 
            className: "meet-experts-section relative isolate overflow-hidden bg-[#FFF8F5] py-8 md:py-24" 
          },
          el(
            "span",
            {
              className: "absolute inset-0 pointer-events-none opacity-70 bg-cover meet-experts-texture",
              style: (function() {
                ...
                if (typeof mainTheme !== 'undefined' && mainTheme.imageUrl) {
                  ...
                  var apiRoot = typeof wpApiSettings !== 'undefined' ? wpApiSettings.root : '';
                  if (apiRoot) {
                    imageUrl = apiRoot.replace('/wp-json/', '/wp-content/themes/main/assets/images/');
                  } else {
                    var origin = window.location.origin;
                    var path = window.location.pathname;
                    path = path.replace(/\/wp-admin.*$/, '').replace(/\/.*$/, '');
                    imageUrl = origin + path + '/wp-content/themes/main/assets/images/';
                  }
                }
                
                return {
                  "--texture-mobile": "url('" + imageUrl + "texture-mobile.svg')",
                  "--texture-desktop": "url('" + imageUrl + "texture-desktop.svg')",
          ...
          el(
            "div",
            { className: "container-1056 relative flex flex-col items-center gap-8 md:gap-14" },
            el(
              "div",
              {
                className: "flex flex-col items-center text-center gap-3 md:gap-3 max-w-[22.75rem] md:max-w-4xl pt-8 pb-4 md:py-0"
              },
              el(RichText, {
                tagName: "h2",
                className: "text-3xl md:text-5xl font-bold font-gilroy text-gray-900 leading-none md:leading-none",
                value: heading,
                onChange: function (value) {
                  setAttributes({ heading: value });
                },
              ...
              el(RichText, {
                tagName: "div",
                className: "text-sm md:text-base font-medium text-gray-500 tracking-2p md:tracking-1p",
                value: subheading,
                onChange: function (value) {
                  setAttributes({ subheading: value });
                },
            ...
            authors.length > 0 &&
              el(
                ...
                el(
                  "div",
                  Object.assign({
                    className: showCarousel ? "-mx-6 -my-12 md:-m-12 meet-experts-carousel" : ""
                  }, showCarousel ? {
                  ...
                  authorGroups.map(function (group, groupIndex) {
                    return el(
                      ...
                      {
                        key: groupIndex,
                        className: "w-full flex flex-col gap-4 px-6 py-12 md:p-12 md:max-w-none md:grid md:grid-cols-2 md:gap-8"
                      },
                      group.map(function (authorId, authorIndex) {
                        var author = getAuthorData(authorId);
                        if (!author) {
                          return el("div", { key: authorIndex }, __("Loading author...", "main"));
                        }
                        ...
                        var authorName = author.name || author.slug || "Unknown";

                        return el(
                          "article",
                          {
                            key: authorIndex,
                            className: "flex flex-col gap-4 md:gap-4 rounded-2xl bg-white p-6 md:p-6 shadow-[0px_56px_23px_rgba(191,191,191,0.01),0px_32px_19px_rgba(191,191,191,0.05),0px_14px_14px_rgba(191,191,191,0.09),0px_4px_8px_rgba(191,191,191,0.1)]"
                          },
                          el(
                            "div",
                            ...
                            el(
                              ...
                              el("h3", {
                                className: "text-base md:text-lg font-semibold text-gray-900 md:-tracking-2p"
                              }, authorName),
                              el("div", {
                                className: "text-xs md:text-sm font-medium text-gray-500 tracking-2p md:tracking-1p"
                              }, __("Author info", "main"))
                            )
                          ),
                          bio &&
                            el("div", {
                              className: "text-sm font-medium text-gray-500 leading-5 tracking-2p md:tracking-1p"
                            }, bio.substring(0, 150) + (bio.length > 150 ? "..." : "")),
                          el("a", {
                            href: archiveUrl,
                            className: "text-sm font-semibold text-primary hover:text-primary-hover md:py-3"
                          }, "Read Article")
                        );
                      })
                    );
