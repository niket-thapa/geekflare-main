  ...
  registerBlockType("main/key-features", {
    edit: function (props) {
      ...
      function addFeature(indexToInsertAfter) {
        var newFeatures = features.slice();
        var insertIndex = indexToInsertAfter !== undefined ? indexToInsertAfter + 1 : newFeatures.length;
            ...
            features.map(function (feature, index) {
              return el(
                "div",
                {
                  ...
                  className:
                    "key-feature-badge box-border flex flex-row items-center py-1.5 px-2.5 md:px-3 gap-2 bg-gray-50 border border-gray-200 rounded-full flex-none group",
                },
                el(
                  RichText,
                  {
                    tagName: "span",
                    className:
                      "text-sm md:text-base font-medium leading-4.5 md:leading-5.5 tracking-2p text-gray-800 flex-none whitespace-nowrap",
                    value: feature,
                ...
                el(
                  Button,
                  {
                    className: "key-features-remove-button opacity-0 group-hover:opacity-100 transition-opacity",
                    isDestructive: true,
                    isSmall: true,
                    onClick: function () {
                      removeFeature(index);
