...
// Get current category if this is a category archive
...
<main id="main-content" class="site-main">
	<?php if ( have_posts() ) : ?>
							...
							<article class="flex flex-col gap-6">
								<div class="relative rounded-2xl overflow-hidden">
									<?php if ( has_post_thumbnail( $sticky_post->ID ) ) : ?>
										<a href="<?php echo esc_url( get_permalink( $sticky_post->ID ) ); ?>">
								...
								<div class="flex flex-col gap-4">
									...
									<div class="flex flex-col gap-4 md:gap-2">
										<h3 class="text-xl md:text-2xl md:leading-8 font-semibold md:line-clamp-1">
								...
								<a href="<?php echo esc_url( get_permalink( $sticky_post->ID ) ); ?>" class="text-sm font-semibold text-primary hover:text-primary-hover lg:hidden">
									Read Guide
								</a>
							</article>
						</div>
		...
		<div class="container-1056 flex flex-col gap-y-12 md:gap-y-14 py-14 md:py-20 xl:py-24">
					...
					data-author-id="<?php echo $current_author_id ? esc_attr( $current_author_id ) : ''; ?>">
						<div class="relative w-full md:max-w-[21.25rem] md:w-[21.25rem]">
							...
							<input
								...
								id="articlesSearch"
								name="archive_search"
								class="form-input h-12 md:h-14 ps-11 pe-4 md:ps-14 tracking-2p"
			...
			<?php
			// Get tags from posts in the current category
			$category_tags = array();
			$current_tag_id = null;
			
			if ( is_category() && $current_category ) {
				// Get all tags used in posts from this category
				...
				if ( ! empty( $tags ) && ! is_wp_error( $tags ) ) {
					// Filter tags to only include those with posts in the current category
					foreach ( $tags as $tag ) {
						// Count posts with this tag in the current category
						...
						if ( $tag_posts->found_posts > 0 ) {
							$category_tags[] = array(
								'term'  => $tag,
								'count' => $tag_posts->found_posts,
							);
						}
				...
				// If viewing a tag archive, get the current tag
			...
			// Only show filter if we have tags
			if ( ! empty( $category_tags ) ) :
				$current_category_url = is_category() ? get_category_link( $current_category->term_id ) : '';
					...
					<div class="tags-filter flex gap-2.5 overflow-x-auto scrollbar-hide">
						<?php if ( is_category() ) : ?>
							<a
								...
								class="tag-item text-sm whitespace-nowrap font-semibold text-gray-500 gap-1.5 rounded-xl md:rounded-2xl border border-gray-200 py-3 px-3.5 md:text-base md:leading-5.5 md:px-4 <?php echo ! $is_tag_archive ? 'active' : ''; ?> [&.active]:bg-primary [&.active]:text-white [&.active]:border-primary hover:bg-primary hover:text-white hover:border-primary transition-colors duration-300">
						...
						<?php foreach ( $category_tags as $tag_data ) : 
							$tag = $tag_data['term'];
							$tag_count = $tag_data['count'];
							$tag_url = get_term_link( $tag );
							$is_active = $is_tag_archive && $current_tag_id === $tag->term_id;
							?>
							<a
								href="<?php echo esc_url( $tag_url ); ?>"
								class="tag-item text-sm whitespace-nowrap font-semibold text-gray-500 gap-1.5 rounded-xl md:rounded-2xl border border-gray-200 py-3 px-3.5 md:text-base md:leading-5.5 md:px-4 <?php echo $is_active ? 'active' : ''; ?> [&.active]:bg-primary [&.active]:text-white [&.active]:border-primary hover:bg-primary hover:text-white hover:border-primary transition-colors duration-300">
								<span class="tags-filter-item-name"><?php echo esc_html( $tag->name ); ?></span>
								<span class="tags-filter-item-count text-xs tracking-2p md:text-sm">
									(<?php echo esc_html( number_format_i18n( $tag_count ) ); ?>)
								</span>
							</a>
						<?php endforeach; ?>
					</div>
